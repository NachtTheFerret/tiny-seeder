...
describe('seeder tool', () => {
  ...
  it('missing properties', () => {
    ...
    expect(() => seeder.add({ name: '', rows: 0, columns: {} })).toThrow('table name must be a string');
    expect(() => seeder.add({ name: 'toto', rows: 0, columns: {} })).toThrow('table "toto" :: rows must be a number');
    expect(() => seeder.add({ name: 'toto', rows: 4, columns: <Table['columns']>(<unknown>null) })).toThrow(
  ...
  it('unknown values', () => {
    ...
    expect(() => seeder.add({ ...table, columns: { key: { nullable: true, unique: true } } })).toThrow(
      'key "key" on table "toto" has no type for data generator'
    );
    expect(() => seeder.add({ ...table, columns: { key: { nullable: true, primary: true } } })).toThrow(
